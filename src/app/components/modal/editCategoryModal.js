...
const EditCategoryModal = ({isOpen, onClose, data, onUpdateCategory}) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const result = await dispatch(categoryUpdateApi({id: data.id, credentials: values}));
            if (categoryUpdateApi.fulfilled.match(result)) {
                formik.resetForm();  // Reset form fields
                onUpdateCategory(result.payload);  // Update the category in the parent component
                Swal.fire({
                    title: 'Success!',
                ...
                Swal.fire({
                    title: 'Error!',
                    text: 'There was an issue updating Category',
                    icon: 'error',
                    confirmButtonText: 'OK'
                });
            }
